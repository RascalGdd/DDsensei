...
class Unpaired_model(nn.Module):
    def __init__(self, opt):
        ...
        if opt.phase == "train":
            ...
            if opt.netDu == 'projected_gan':
                self.netDu = discriminators.Projectedgan_Discriminator(opt)
                paralist = list(self.netDu.f1.parameters()) + list(self.netDu.f2.parameters()) + list(self.netDu.f3.parameters()) + list(
                    self.netDu.f4.parameters())
    ...
    def forward(self, image, label, mode, losses_computer):
        ...
        if mode == "losses_G":
            ...
            pred_fake1, pred_fake2, pred_fake3, pred_fake4 = self.netDu(fake)
            lst0 = [pred_fake1, pred_fake2, pred_fake3, pred_fake4]
            for pred_fake in lst0:
        ...
        if mode == "losses_Du":
            ...
            with torch.no_grad():
                fake = self.netG(label,edges = edges)
            output_Du_fake1, output_Du_fake2, output_Du_fake3, output_Du_fake4 = self.netDu(fake)
            lst = [output_Du_fake1, output_Du_fake2, output_Du_fake3, output_Du_fake4]
            for output_Du_fake in lst:
                loss_Du_fake = self.criterionGAN(output_Du_fake, False).mean()
                loss_Du += loss_Du_fake

            output_Du_real1, output_Du_real2, output_Du_real3, output_Du_real4 = self.netDu(image)
            lst2 = [output_Du_real1, output_Du_real2, output_Du_real3, output_Du_real4]
            for output_Du_real in lst2:
