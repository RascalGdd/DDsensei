...
class CNNBlock(nn.Module):
    def __init__(self, opt, in_channels, out_channels, stridesize=2):
        super().__init__()
        norm_layer = norms.get_spectral_norm(opt)
        self.conv = nn.Sequential(
            norm_layer(nn.Conv2d(in_channels, out_channels, kernel_size=(4, 4), stride=(stridesize, stridesize), bias=False, padding=1, padding_mode="reflect")),
...
effnet = torchvision.models.efficientnet_b0(pretrained=True,progress=True)
for para in effnet.parameters():
    para.requires_grad = False
class Feature_extract(nn.Module):
    def __init__(self):
        super().__init__()
        self.f1 = nn.Sequential(*effnet.features[:3])
        self.f2 = nn.Sequential(*effnet.features[:4])
        self.f3 = nn.Sequential(*effnet.features[:5])
        self.f4 = nn.Sequential(*effnet.features[:8])
...
class Feature_translate(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        self.conv = nn.Conv2d(in_channels=in_channels, out_channels=in_channels,stride=1,kernel_size=1)
...
class Feature_discriminator(nn.Module):
    def __init__(self, opt, size, in_channels):
        super().__init__()
        norm_layer = norms.get_spectral_norm(opt)
        num_layers =int(np.log2(size/4))
        layers = nn.ModuleList()
        for i in range(num_layers):
            layers.append(CNNBlock(opt, in_channels, in_channels))
        layers.append(norm_layer(nn.Conv2d(in_channels, 1, 1, stride=1,padding_mode="reflect")))
...
class Projectedgan_Discriminator(nn.Module):
    def __init__(self, opt):
        super().__init__()
        self.feature_extract = Feature_extract()
        self.f1 = Feature_translate(in_channels=24)
        self.f2 = Feature_translate(in_channels=40)
        self.f3 = Feature_translate(in_channels=80)
        self.f4 = Feature_translate(in_channels=320)
        self.d1 = Feature_discriminator(opt=opt, in_channels=24, size=64)
        self.d2 = Feature_discriminator(opt=opt, in_channels=40, size=32)
        self.d3 = Feature_discriminator(opt=opt, in_channels=80, size=16)
        self.d4 = Feature_discriminator(opt=opt, in_channels=320, size=8)
        # network = nn.Sequential(
        #     self.feature_extract, self.f1, self.f2, self.f3, self.f4, self.d1, self.d2, self.d3, self.d4
        # )
    def forward(self, x):
        a, b, c, d = self.feature_extract(x)
        a = self.d1(self.f1(a))
        b = self.d2(self.f2(b))
        c = self.d3(self.f3(c))
        d = self.d4(self.f4(d))
