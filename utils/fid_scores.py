...
class fid_pytorch_test():
    def __init__(self, opt, dataloader_val, gta_val):
        self.opt = opt
        self.dims = 2048
        block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[self.dims]
        self.model_inc = InceptionV3([block_idx])
    ...
    def compute_fid_with_valid_path(self):
        pool, logits, labels = [], [], []
        self.model_inc.eval()
        with torch.no_grad():
    ...
    def update(self):
        cur_fid = self.compute_fid_with_valid_path()
