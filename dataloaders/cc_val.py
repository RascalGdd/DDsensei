...
class CC_VAL(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        image = Image.open(os.path.join(self.images[idx%len(self.images)])).convert('RGB')
        label = Image.open(os.path.join(self.labels[idx%len(self.labels)]))
    ...
    def list_images(self):
        images = []
        labels = []
        path_img = os.path.join(self.opt.dataroot, "leftImg8bit", "val")
