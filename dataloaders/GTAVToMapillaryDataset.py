...
mapillary_training_img_path = r"/data/public/mapillary/training/images"
mapillary_validation_img_path = r"/data/public/mapillary/validation/images"
mapillary_validation_lbl_path = r"/no_backups/s1422/output_labels/validation/id"

class GTAVToMapillaryDataset(torch.utils.data.Dataset):
    ...
    def list_images(self):
        ...
        if mode == "val":
            images = []
            labels = []
            path_img = os.path.join(mapillary_validation_img_path)
            for item in sorted(os.listdir(path_img)):
                images.append(os.path.join(item))

            path_lab = os.path.join(mapillary_validation_lbl_path)
            for item in sorted(os.listdir(path_lab)):
                labels.append(os.path.join(item))
...
class GTAVToMapillaryDataset2(torch.utils.data.Dataset):
    def __init__(self, opt, for_metrics,for_supervision = False):
    ...
    def list_images(self):
        ...
        if mode == "train":
            images = []
            path_img = os.path.join(mapillary_training_img_path)
            for item in sorted(os.listdir(path_img)):
                images.append(os.path.join(item))
            labels = []
            images2 = []
