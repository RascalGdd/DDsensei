...
for epoch in range(start_epoch, opt.num_epochs):
    for i, data_i in enumerate(dataloader):
        ...
        model.module.netG.zero_grad()
        loss_G_ori2 = model(image, label, "losses_G_ori2", losses_computer,image2)
        loss_G_ori2 = loss_G_ori2.mean()
        loss_G_ori2.backward()
        optimizerG.step()

        model.module.netG.zero_grad()
        loss_G_ori1 = model(image, label, "losses_G_ori1", losses_computer,image2)
        loss_G_ori1 = loss_G_ori1.mean()
        loss_G_ori1.backward()
        optimizerG.step()
        ...
        model.module.netD_ori.zero_grad()
        loss_D_ori, losses_D_list_ori = model(image, label, "losses_D_usis", losses_computer)
